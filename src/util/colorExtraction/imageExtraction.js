...
ImageHarmonyMath.weight = function (
    hue,
    saturation,
    value,
    colorfulnessEnhancement,
    shadowHighlightSuppression,
) {
    var minWeight = 0.001
    var weightNew =
        ImageHarmonyMath.suppression(
            ImageHarmonyMath.colorfulness(hue, saturation, value),
            colorfulnessEnhancement,
        ) *
        ImageHarmonyMath.suppression(value, shadowHighlightSuppression) *
        ImageHarmonyMath.tripleCubicSigmoid(saturation, 0.0, 30.0) *
        ImageHarmonyMath.tripleCubicSigmoid(value, 0.0, 30.0)
    return weightNew < minWeight ? minWeight : weightNew
}

ImageHarmonyMath.colorfulness = function (hue, saturation, value) {
    var lab = color.rgbToLab(
        color.hsvToRgb({ h: hue / 255, s: saturation / 255, v: value / 255 }),
    )
    var a = lab.a * 2.0 - 1.0
    var b = lab.b * 2.0 - 1.0
...
ImageHarmonyMath.suppression = function (value, amount) {
    ...
    return Math.pow(
        f * ImageHarmonyMath.cubicSigmoid(s) + (1.0 - f),
        1.0 + g * 3.0,
    )
}
...
var epsilon = 1.0e-10,
    hardCoreEnergy,
    d
var practicallyInfiniteEnergy = 1.79769e305 //Math.pow(2,64)/1000.0;
function intersectionEnergy(
    p1,
    p2,
    hardCoreInteractionRadius,
    colorRepulsion,
    gBB,
    gSS,
) {
    ...
    hardCoreEnergy =
        d < hardCoreInteractionRadius ? practicallyInfiniteEnergy : 0
    ...
    return d < hardCoreInteractionRadius
        ? hardCoreEnergy * (1.0 - d / hardCoreInteractionRadius)
        : colorRepulsion * (1.0 / d - 1.0 / hardCoreInteractionRadius)
}
...
ImageHarmony.prototype.extractColors = function (
    bitmapData,
    colorStyle,
    numColors,
) {
...
ImageHarmony.prototype.weightAndNormalize = function () {
    ...
    for (h = 0; h < nh; h++) {
        ...
        for (s = 0; s < ns; s++) {
            ...
            for (v = 0; v < nv; v++) {
                ...
                density = histogram[(h * NUM_BINS_S + s) * NUM_BINS_V + v] *=
                    ImageHarmonyMath.weight(
                        hue,
                        saturation,
                        value,
                        colorfulnessEnhancement,
    ...
    var maxLogDensity =
        maxDensity > smallestAllowedDensity
            ? Math.log(maxDensity / smallestAllowedDensity)
            : 0
        ...
        for (h = 0; h < nh; h++) {
            for (s = 0; s < ns; s++) {
                for (v = 0; v < nv; v++) {
                    ...
                    logDensity =
                        density > smallestAllowedDensity
                            ? Math.log(density / smallestAllowedDensity) /
                              maxLogDensity
                            : 0
                    if (logDensity !== 0) {
                        volume++
    ...
    for (h = 0; h < nh; h++) {
        ...
        for (s = 0; s < ns; s++) {
            ...
            for (v = 0; v < nv; v++) {
                ...
                color = {
                    hsv: { h: hue / 360, s: saturation / 100, v: value / 100 },
                }
...
ImageHarmony.prototype.findPixel = function (color) {
    ...
    //TODO: Needs Review.
    var smallestDistance = 1.79769e305 //Math.pow(2,31) - 1;
...
/**************************************************************************************/
export function extractColorFromImage(imageData, width, height, count, mood) {
...
function findPixels(imageData, colors) {
    var harmony = new ImageHarmony()
    harmony._bitmapData = new BitmapData(
        imageData,
        imageData.width,
        imageData.height,
    )
